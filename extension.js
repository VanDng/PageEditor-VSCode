...
async function InitializeWorkingDirectory(isAutoInitialization = false) {
    ...
    } else if (IsEmptyDirectory() == false && isAutoInitialization == false) {
    ...
    IntervalExecutor.Execute(5000, async function() {
        ...
        if (IsPageEditorWorkspace()) {
            if (IsSingleWorkspace()) {
                if (!_isExtensionStarted) {
                    await InitializeWorkingDirectory(true);
                    StartExtension(true);
                }
            } else {
                if (_isExtensionStarted) {
